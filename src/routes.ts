...
import type { HostServices, StartBody, RestartBody } from "./types.js";
import {
  checkInstallation,
  installCodeServer,
  getBinaryPath,
...
export function createCodeServerRoutes(hostServices: HostServices) {
  const { storage } = hostServices;

  return (
    new Elysia({ prefix: "/api/code-server" })
